...
test('app shell renders imprint footer with version binding', async () => {
    ...
    const footerSloganStyles = extractCssBlock(styles, '.footer-slogan')

    assert.match(source, /<footer class="page-footer"/)
    assert.match(source, /class="footer-card"/)
    assert.match(source, />\s*Imprint\s*</)
    assert.match(source, /class="footer-slogan"/)
    assert.match(source, />\s*Build and hostet in Germany\s*</)
    ...
    assert.match(footerSloganStyles, /justify-self:\s*center/)
    assert.match(footerSloganStyles, /text-align:\s*center/)
